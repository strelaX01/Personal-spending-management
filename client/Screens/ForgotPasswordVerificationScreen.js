...
const ForgotPasswordVerificationScreen = ({ navigation, route }) => {
    ...
    const handleVerify = async () => {
        ...
        try {
            const response = await fetch('http://10.0.2.2:3000/api/auth/ForgotPasswordverify', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                setLoading(false);
                setModalMessage('Xác minh thành công, mật khẩu mới đã được gửi đến email của bạn.');
                ...
                // Không reset timer ở đây để nó tiếp tục đếm ngược
            }
        } catch (error) {
            setLoading(false);
    ...
    const handleResendCode = async () => {
        ...
        try {
            const response = await fetch('http://10.0.2.2:3000/api/auth/ForgotPasswordResend', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (response.ok && data.success) {
                ...
                setErrorModalMessage(data.message || 'Có gì đó không ổn. Vui lòng thử lại.');
                setErrorModal(true);
                setIsResendEnabled(true);
            }
        } catch (error) {
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View style={styles.innerContainer}>
                    ...
                    <Text style={styles.subtitle}>Chúng tôi đã gửi mã xác minh đến email của bạn.</Text>
                    ...
                    <Text style={styles.expiresText}>
                        Mã hết hạn trong: {Math.floor(codeValidityTimer / 60)}:{String(codeValidityTimer % 60).padStart(2, '0')}
                    </Text>
