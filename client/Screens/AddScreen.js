...
const AddScreen = () => {
  ...
  const fetchCategories = async (type) => {
    try {
      const endpoint = type === 'Income' ? 'GetIncomeCategories' : 'GetExpenseCategories';
      const response = await axios.get(`http://10.0.2.2:3000/api/category/${endpoint}?token=${token}`);
...
>>>>>>> eace508c10eced53687afe40a8a2bbaaa287535a
      ...
      if (error.response) {
        ...
        if (error.response.status === 400 && error.response.data.requireConfirmation) {
        ...
        console.error('Lỗi khi thêm thu nhập:', error.message);
        setErrorModalMessage('Lỗi khi thêm thu nhập');
        setErrorModal(true);
      }
    } finally {
...
>>>>>>> eace508c10eced53687afe40a8a2bbaaa287535a
        ...
        if (error.response) {
            ...
            setErrorModalMessage('Không thể kết nối đến server. Vui lòng kiểm tra kết nối mạng và thử lại.');
            setErrorModal(true);
        } else {
            // Lỗi khác (có thể do cấu hình request hoặc client-side)
            console.error('Lỗi:', error.message);
            setErrorModalMessage('Đã xảy ra lỗi không xác định. Vui lòng thử lại sau.');
            setErrorModal(true);
        }
    } finally {
        setIsSaving(false);
...
>>>>>>> eace508c10eced53687afe40a8a2bbaaa287535a
  ...
  const fetchExpenseData = useCallback(async (date) => {
    ...
    try {
      ...
      const response = await fetch(`http://10.0.2.2:3000/api/transaction/getExpense?token=${token}&monthYear=${monthYear}`);
  ...
  const fetchIncomeData = useCallback(async (date) => {
    ...
    try {
      const monthYear = `${String(date.getMonth() + 1).padStart(2, '0')}-${date.getFullYear()}`;
      const response = await fetch(`http://10.0.2.2:3000/api/transaction/getIncome?token=${token}&monthYear=${monthYear}`);
      const data = await response.json();
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.changeAddButton} onPress={openModal}>
        <Text style={styles.buttonText}>{selectedType === 'Income' ? 'Thu nhập' : 'Chi tiêu'}</Text>
        <MaterialIcons
      ...
      {modalVisible && (
        <TouchableWithoutFeedback onPress={closeModal}>
          <View style={styles.modalOverlay}>
            <Animated.View
              style={[styles.dropdown, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>

              <TouchableOpacity
                style={[styles.optionButton, selectedType === 'Income' && styles.selectedOption]}
                onPress={() => selectType('Income')}
              >
                <Text style={styles.optionText}>Thu nhập</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.optionButton, selectedType === 'Expense' && styles.selectedOption]}
        ...
        {selectedType === 'Income' && (
          <View style={styles.incomeContainer}>
            ...
            <View style={styles.noteBookContainer}>
              <MaterialCommunityIcons style={styles.noteBookIcon} name="notebook-minus-outline" size={30} color="black" />
              <TextInput
          ...
          <View style={styles.modalView}>
            ...
            <View style={styles.modalButtons}>
              ...
              <TouchableOpacity
                ...
                onPress={selectedType === 'Income' ? confirmIncome : confirmExpense}
              >
