import { StyleSheet, Text, View, FlatList, TouchableOpacity, ScrollView, KeyboardAvoidingView, Platform } from 'react-native';
...
const DetailReportYearScreen = () => {
    ...
    const chartConfig = {
        ...
        formatYLabel: (value) => parseInt(value).toLocaleString('vi-VN'),
        formatTopBarValue: (value) => parseInt(value).toLocaleString('vi-VN'),
    };
    ...
    const totalAmountColor = totalAmount < 0 ? 'red' : totalAmount > 0 ? '#008B45' : 'white';
    const totalAmountText = totalAmount.toLocaleString('vi-VN');
    const totalAmountDisplay = totalAmount > 0 ? `+${totalAmountText}` : totalAmountText;

    const year = filteredData.length > 0 ? filteredData[0].date.split('-')[2] : '';
        ...
        <View style={styles.container}>
            ...
            <View style={styles.bodyContainer}>
                <Text style={styles.totalText}>
                    Tổng cộng: <Text style={{ color: totalAmountColor }}>{totalAmountDisplay}</Text>
                </Text>
