  ...
  const fetchData = useCallback(async (endpoint, params = {}) => {
    ...
    try {
      ...
      if (['getExpense', 'getIncome', 'getAnnualExpense', 'getAnnualIncome'].includes(endpoint)) {
        prefix = 'transaction';
      } else if (['getMonthlyExpensePlan', 'getMonthlyIncomePlan', 'getAnnualExpensePlan', 'getAnnualIncomePlan'].includes(endpoint)) {
        prefix = 'plan';
      }

      const queryParams = new URLSearchParams(params).toString();
      const response = await fetch(`${baseUrl}/${prefix}/${endpoint}?token=${token}&${queryParams}`);
...
const ReportScreen = () => {
  ...
  const { calculateTotalAmount, formatMonthYear, fetchData } = useFinanceApi(token);
  ...
  const fetchExpenseData = useCallback(async (date) => {
    ...
    try {
      ...
      const response = await fetchData('getExpense', { monthYear });
  ...
  }, [token, formatMonthYear, fetchData, calculateTotalAmount]);
  ...
  const fetchIncomeData = useCallback(async (date) => {
    ...
    try {
      const monthYear = formatMonthYear(date);
      const response = await fetchData('getIncome', { monthYear });
  ...
  const fetchAnnualExpenseData = useCallback(async (date) => {
    ...
    try {
      const response = await fetchData('getAnnualExpense', { year: date.getFullYear() });
      const expenses = response.expenses || [];
      setExpenseData(expenses);
  ...
  }, [token, fetchData, calculateTotalAmount, calculateAnnualCategoryData]);

  const fetchAnnualIncomeData = useCallback(async (date) => {
    if (!token) return;

    try {
      const response = await fetchData('getAnnualIncome', { year: date.getFullYear() });
      const incomes = response.incomes || [];
      setIncomeData(incomes);
  ...
  const fetchPlanData = useCallback(async (date) => {
    ...
    try {
      const monthYear = formatMonthYear(date);
      const expensePlanResponse = await fetchData('getMonthlyExpensePlan', { monthYear });
      const incomePlanResponse = await fetchData('getMonthlyIncomePlan', { monthYear });
      const annualExpensePlanResponse = await fetchData('getAnnualExpensePlan', { year: date.getFullYear() });
      const annualIncomePlanResponse = await fetchData('getAnnualIncomePlan', { year: date.getFullYear() });

      setExpensePlanData(expensePlanResponse.expensePlan || []);
      setIncomePlanData(incomePlanResponse.incomePlan || []);
      setAnnualExpenseCategories(annualExpensePlanResponse.expensePlan || []);
      setAnnualIncomeCategories(annualIncomePlanResponse.incomePlan || []);
    } catch (error) {
      setExpensePlanData([]);
      setIncomePlanData([]);
      setAnnualExpenseCategories([]);
  ...
  }, [token, formatMonthYear, fetchData]);
