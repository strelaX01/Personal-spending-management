...
const IncomeCategoryHome = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://10.0.2.2:3000/api/category/GetIncomeCategories?token=${token}`
      );
  ...
  const handleEdit = useCallback((item) => {
    ...
    setTimeout(() => {
      navigation.navigate('EditIncomeCategory', { category: item });
    }, 300); 
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `http://10.0.2.2:3000/api/category/DeleteIncomeCategory/${categoryToDelete.category_id}?token=${token}`
      );
      ...
      {loading ? (
        ...
        <FlatList
          ...
          keyExtractor={(item, index) =>
            item.category_id ? item.category_id.toString() : `temp-${index}`
          }
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            <Text style={styles.modalText}>Bạn có chắc chắn muốn xóa danh mục không? "{categoryToDelete?.category_name}"?</Text>
...
export default IncomeCategoryHome;
