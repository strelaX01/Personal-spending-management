...
const ExpenseCategoryHome = () => {
  ...
  const fetchCategories = async (token) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://10.0.2.2:3000/api/category/GetExpenseCategories?token=${token}`
      );
  ...
  const handleEdit = useCallback((item) => {
    ...
    // Use setTimeout to ensure the swipeable is closed before navigation
    setTimeout(() => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await axios.delete(
        `http://10.0.2.2:3000/api/category/DeleteExpenseCategory/${categoryToDelete.category_id}?token=${token}`
      );
  ...
  const renderRightActions = (item, progress) => {
    ...
    return (
      <View style={styles.actionContainer}>
        ...
        <Animated.View style={{ transform: [{ translateX: trans }], opacity, marginLeft: 5 }}>
          <TouchableOpacity
    ...
    return (
      ...
      >
        <TouchableOpacity style={styles.categoryContainer}
          onPress={() => {
            // Close this swipeable if it's open
          ...
          >
            <Ionicons name={item.category_icon} size={26} color={item.category_color} />
          </View>
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          // Close all swipeables when the list is scrolled
          onScroll={() => {
            closeAllSwipeables();
          }}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalView}>
            <Text style={styles.modalText}>Bạn có chắc chắn muốn xóa danh mục "{categoryToDelete?.category_name}"?</Text>
