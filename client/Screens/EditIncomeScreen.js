  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://10.0.2.2:3000/api/finance/income/categories`,
        {
...
const handleEdit = async () => {
  ...
  try {
    ...
    const response = await axios.put(
      `http://10.0.2.2:3000/api/finance/income/${data.id}`,
      updatedIncome,
      {
        headers: {
    ...
    // 🔸 Nếu server yêu cầu xác nhận (chưa có kế hoạch hoặc kế hoạch = 0)
    if (respData.requireConfirmation) {
      setModalMessage(respData.message);
    ...
    if (respData.success) {
      setConfirmationMessage(respData.message || "Cập nhật thu nhập thành công");
      setConfirmationModalVisible(true);
    } else {
      setErrorModalMessage(respData.message || "Có lỗi xảy ra khi cập nhật thu nhập");
      setErrorModal(true);
    }
  ...
  const confirmEdit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://10.0.2.2:3000/api/finance/income/confirmEdit/${data.id}`,
        updatedIncome,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      if (response.status === 200) {
        setConfirmationMessage(
          "Thu nhập đã được cập nhật thành công sau khi xác nhận!"
        );
        setConfirmationModalVisible(true);
      } else {
