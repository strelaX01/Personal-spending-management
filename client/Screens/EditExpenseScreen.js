...
const EditExpenseScreen = ({ route }) => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(`http://10.0.2.2:3000/api/category/GetExpenseCategories?token=${token}`);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://10.0.2.2:3000/api/transaction/EditExpense/${data.id}`, updatedExpense);


      if (response.status === 200) {
        setModalMessage('Cập nhật chi tiêu thành công');
  ...
  const confirmEdit = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://10.0.2.2:3000/api/transaction/confirmEditExpense', updatedExpense);
